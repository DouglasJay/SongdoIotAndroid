...
public class ArduinoActivity extends AppCompatActivity {
    ...
    class ItemAdapter extends ArrayAdapter { //Alt+enter 생성자 생성
    ...
    class LoadSensorLogs extends AsyncTask<String, String, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            try {
                String apiURL = "http://192.168.0.12:3000/devices/"+params[0]+"/"+params[1];
        ...
        protected void onPostExecute(String s) {
            dialog.dismiss();
            try {
                JSONArray array = new JSONArray(s); //JSON문자열->JSON객체로 변환
                //String items[] = new String[array.length()];
                items.clear();
                for (int i = 0; i < array.length(); i++) {
                    JSONObject obj = array.getJSONObject(i);
                    items.add(new Item(obj.getInt("temp"), obj.getInt("humidity"), obj.getString("created_at")));
                }
    ...
    class SendBuzzerFlag extends AsyncTask<String, String, String> {
        ...
        protected String doInBackground(String... params) {
            StringBuffer response = new StringBuffer();
            try {
                String urlString = "http://192.168.0.35:3000/devices/buzzer/"+params[0]; //WiFi 연결된 장비 IP
